...
function movedisk(k) //k-th instruction
{
    ...
    if (q.length > 0)
    {
        ...
        x1 = disk1.style.left;
        y1 = disk1.style.top;
        y1 = (parseInt(y1.substring(0, y1.length-2))- THICKNESS+10) + 'px';
...
columns.forEach((column) => {  
    ...
    column.addEventListener('mousedown', (e) => {  
        isDragging = true;  
        initialX = parseInt(window.getComputedStyle(column).getPropertyValue('left'), 10);  
    ...
    window.addEventListener('mousemove', (e) => {  
        ...
        if (currentX > containerRect.width - column.offsetWidth) currentX = containerRect.width - column.offsetWidth;  
